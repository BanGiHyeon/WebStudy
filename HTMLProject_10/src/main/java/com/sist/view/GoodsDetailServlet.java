...
public class GoodsDetailServlet extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      for(ReplyVO rvo:list)
      {
         ...
         out.write("<input type=submit value=\"댓글 수정\" style=\"width:100px;height:95px;background:blue;color:white\">");
         out.write("</form>");
         //처리하는 서블릿(JSP) / 화면출력 서블릿(JSP)
         // => 화면이 없는 (HTML) => 자체에서 처리
         out.write("</td>");
         out.write("</tr>");
         out.write("</table>");
      ...
      if(id!=null) //로그인이 되었다면
      {
         ...
         out.write("<input type=submit value=\"댓글 작성\" style=\"width:100px;height:95px;background:blue;color:white\">");
         out.write("</form>");
      }
      out.write("</div>");
      out.write("</body>");
